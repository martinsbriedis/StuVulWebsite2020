...
_&copy; Martins Briedis, 25/05/2020_
...
- ask Martins [martins.briedis@vogelwarte.ch](mailto:martins.briedis@vogelwarte.ch) to be added as a collaborator to the repo
...
```
git clone --recursive -j8 https://github.com/martinsbriedis/StuVulWebsite2020.git
```

- go into the now created folder (in your explorer) and open the `StuVulWebsite2020.RProj` file in RStudio.
...
- `content`: contains markdown files used to render the page 
	...
	+ `authors`: individual markdown file for each team member 
...
If you have any trouble getting things to work, one way is to post an issue on the github repo or email Martins [martins.briedis@vogelwarte.ch](mailto:martins.briedis@vogelwarte.ch). 
...
You can also add an image to go with the paper by putting it into `static/img/papers/` and linking to by adapting the `image =""` parameter (e.g. `image ="papers/StePar.png"`).
